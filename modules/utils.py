...
расчёт термодинамических свойств, In-Silico PCR (с поддержкой вырожденных праймеров).
"""
import re
import primer3
import pandas as pd
...
# Словарь IUPAC для вырожденных нуклеотидов
IUPAC_CODES = {
    'A': {'A'}, 'C': {'C'}, 'G': {'G'}, 'T': {'T'},
    'R': {'A', 'G'},        # Purine (A or G)
...
def validate_sequence(seq: str) -> Tuple[bool, str, str]:
    ...
    if n_fraction > 0.2: # Смягчили фильтр для вырожденных матриц
...
def calc_gc(seq: str) -> float:
    seq = seq.upper()
    # Для вырожденных праймеров точный GC посчитать сложно, считаем гарантированные G и C
    gc = seq.count("G") + seq.count("C") + seq.count("S")*1.0 + (seq.count("R")+seq.count("Y")+seq.count("K")+seq.count("M"))*0.5
...
def primers_to_dataframe(primers: List[PrimerResult]) -> pd.DataFrame:
    ...
    for p in primers:
        ...
        if getattr(p, "probe_seq", None):
            ...
            row["Проба Tm"] = round(p.probe_tm, 1) if getattr(p, "probe_tm", None) else ""
            row["Проба GC%"] = round(p.probe_gc, 1) if getattr(p, "probe_gc", None) else ""
...
def is_mismatch(template_base: str, primer_base: str) -> bool:
    """
    Проверяет, является ли пара оснований мисматчем с учетом вырожденных кодов (IUPAC).
    """
    if template_base == primer_base:
        return False
        
    # Получаем возможные нуклеотиды для буквы праймера (например, R -> {'A', 'G'})
    valid_bases_for_primer = IUPAC_CODES.get(primer_base, {primer_base})
    
    # Если нуклеотид матрицы входит в разрешенный набор праймера — это НЕ мисматч
    if template_base in valid_bases_for_primer:
        return False
...
def find_binding_sites(template: str, search_seq: str, max_mismatches: int = 2, is_reverse: bool = False) -> List[Dict]:
    """Ищет места посадки скользящим окном с учетом вырожденных мисматчей."""
    sites = []
    t_len = len(template)
    ...
    for i in range(t_len - p_len + 1):
        ...
        # Интеллектуальный подсчет несовпадений (понимает R, Y, W, S, M, K и т.д.)
        mismatches = sum(1 for t_base, p_base in zip(window, search_upper) if is_mismatch(t_base, p_base))
        
        if mismatches <= max_mismatches:
            if not is_reverse:
                end_mismatches = sum(1 for t_base, p_base in zip(window[-5:], search_upper[-5:]) if is_mismatch(t_base, p_base))
            else:
                end_mismatches = sum(1 for t_base, p_base in zip(window[:5], search_upper[:5]) if is_mismatch(t_base, p_base))
