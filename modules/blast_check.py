...
Оптимизировано для работы в облаке Streamlit (без прерывания WebSockets).
"""
import time
import urllib.error
...
# --- Правило вежливости NCBI (снижает шанс блокировок) ---
NCBIWWW.email = "primer_designer_pro@science.com"
    ...
    for attempt in range(max_retries):
        try:
            if progress_callback and attempt > 0:
                progress_callback(fraction, f"{total_text} (Попытка {attempt+1} из {max_retries}...)")

            entrez_query = f"txid{taxid}[ORGN]" if taxid else ""

            # ВОЗВРАЩЕНО: word_size=7 (самый стабильный метод поиска праймеров в API NCBI)
            ...
            # БЕЗОПАСНЫЙ ПАРСИНГ: Если результатов 0, NCBIXML.parse() отдаёт пустой генератор
            try:
                blast_record = next(blast_records)
            except StopIteration:
                # Результатов просто нет, это не ошибка
                return result

            for alignment in blast_record.alignments:
                title = alignment.title or "Unknown Title"
                
                # Очистка названия гена от мусора NCBI
            ...
            if e.code == 429:
                err_str = "NCBI временно заблокировал IP (Слишком много запросов / 429 Too Many Requests)."
            elif e.code == 403:
                err_str = "NCBI отклонил доступ (403 Forbidden)."
            else:
                err_str = f"HTTP Ошибка {e.code}: {e.reason}"
        ...
        if attempt < max_retries - 1:
            ...
            result.error_message = f"Сбой сети NCBI: {err_str}"
            return result
    ...
    for db in databases:
        for org in organism_groups:
            ...
            for primer_seq, primer_name in [(left_seq, "Forward"), (right_seq, "Reverse")]:
                ...
                time.sleep(3) # Обязательная пауза NCBI
