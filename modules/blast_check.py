...
Содержит ПОЛНЫЙ список таксонов и асинхронное ядро (Polling) 
для обхода зависаний (тайм-аутов) в облаке Streamlit Cloud.
"""
import time
import requests
...
# Полный список групп организмов (Восстановлено на 100%)
...
def run_blast_check(
    ...
    # Параметры инициализации поиска NCBI
    put_params = {
        "CMD": "Put",
        "PROGRAM": "blastn",
        "DATABASE": database,
        "QUERY": primer_seq,
        "SHORT_QUERY": "true", # Обязательно для коротких праймеров
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not rid:
                raise Exception("Не удалось получить RID (ID запроса) от NCBI.")

            # 2. Асинхронное ожидание (Anti-Timeout система)
            # Эта часть не даёт облаку Streamlit оборвать соединение с вашим браузером
            poll_params = {"CMD": "Get", "FORMAT_OBJECT": "SearchInfo", "RID": rid}
            elapsed_total = 0
            xml_data = None

            while True:
                # NCBI просит ждать 15 сек перед первым пингом, и 30 сек между остальными
                wait_step = 15 if elapsed_total == 0 else 30
                
                # Дробим ожидание на отрезки по 5 секунд для обновления UI
                for _ in range(wait_step // 5):
                    time.sleep(5)
                    elapsed_total += 5
                    if progress_callback:
                        # Постоянно обновляем экран, чтобы Streamlit не "убил" вкладку
                ...
                if "Status=WAITING" in poll_text:
                    ...
                    if "<html" in poll_text.lower():
                        raise Exception("NCBI вернул HTML-страницу ошибки. Возможна временная блокировка IP.")
                    continue

            # Если хитов нет, выходим без скачивания
            if xml_data == "": 
                return result

            # 3. Скачиваем готовый XML результат
            if progress_callback:
