...
Финальная мастер-версия: сохранены ВСЕ оригинальные функции (экспорт проб, 
полные таблицы без обрезания колонок) + вшита умная обработка BLAST.
"""
...
if mode == "Анализ олигонуклеотида":
    ...
    with tab_blast:
        ...
        if blast_left_seq and blast_right_seq:
            ...
            if st.button("🔍 Запустить BLAST", type="primary", use_container_width=True):
                # ЗАЩИТА ОТ ТИХОЙ ОШИБКИ NCBI (Игнорируют короткие запросы)
    ...
    with tab_results:
        ...
        if not st.session_state.primers:
            ...
            # ВОССТАНОВЛЕН МИНИМАЛЬНЫЙ ФОРМАТ ЭКСПОРТА ПРОБЫ
            col1, col2, col3 = st.columns(3)
