...
Полная версия + Вывод генов-мишеней BLAST + Интерактивная векторная карта экзонов.
"""
...
def add_exons_to_map(fig, exons_1b):
    ...
    # Чередующиеся цвета для лучшей видимости границ экзонов
    bg_colors = ["#e0f2f1", "#b2dfdb"] 
    for idx, (s1, e1) in enumerate(exons_1b):
        c = bg_colors[idx % 2]
        fig.add_vrect(
            x0=s1, x1=e1, 
            fillcolor=c, opacity=0.6, 
            line_width=1.5, line_color="#00695c", 
            annotation_text=f"<b>Экзон {idx+1}</b>", annotation_position="top left",
            annotation_font_size=11, annotation_font_color="#004d40"
        )
                            ...
                            for p in primers:
                                for a in ann_filtered:
                                    if p.pair_id == a["pair_id"]:
                                        # ПРОБРАСЫВАЕМ В ОБЪЕКТ ДАННЫЕ О СТЫКЕ ДЛЯ ВИЗУАЛИЗАТОРА
        ...
        if st.session_state.blast_results:
            ...
            for key, pair_results in st.session_state.blast_results.items():
                ...
                for res in pair_results:
                    # ВЫВОД ИНФОРМАЦИИ И ГЕНОВ-МИШЕНЕЙ
                    ...
                    # Показываем список амплифицируемых генов!
                    if res.target_genes:
                        st.markdown(f"**🎯 Амплифицируемые гены (мишени):**")
                        for g in sorted(res.target_genes):
                            st.markdown(f"- `{g}`")
...
with tab_results:
    ...
    if not st.session_state.primers:
        ...
        if pairs_to_show:
            # ОТРИСОВКА КАРТЫ С ЭКЗОНАМИ И СТРЕЛКАМИ
