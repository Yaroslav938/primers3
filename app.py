...
def add_exons_to_map(fig, exons_1b):
    for idx, (s1, e1) in enumerate(exons_1b, start=1):
        fig.add_vrect(x0=s1-1, x1=e1, fillcolor="lightgreen", opacity=0.15, line_width=1, line_color="green", annotation_text=f"Exon {idx}", annotation_position="top left")
    for j in exon_junctions_0based(exons_1b):
        fig.add_vline(x=j, line_width=1, line_dash="dot", line_color="green")
    return fig
    ...
    if seq_input:
        ...
        if not is_valid:
            ...
            if st.session_state.primers_df is not None and not st.session_state.primers_df.empty:
                ...
                # Стандартизируем названия колонок для обработки (защита от разных версий)
                ...
                # Собираем список колонок, которые мы ХОТИМ показать
                cols_to_keep = [
                ...
                # Оставляем только те колонки, которые реально существуют в df_disp
                final_cols = [c for c in cols_to_keep if c in df_disp.columns]
                df_disp = df_disp[final_cols]
                
                # Переименовываем для финального отображения
                final_rename_map = {
                ...
                # Рендер таблицы с форматированием новых колонок
                st.dataframe(
                    df_disp,
            ...
            for key, pair_results in st.session_state.blast_results.items():
                ...
                for res in pair_results:
                    if res.error_message:
                        ...
                        continue
                    if res.warning_message: st.warning(f"**{res.primer_name}**: {res.warning_message}")
                    else: st.success(f"**{res.primer_name}**: Высокоспецифичен.")
