...
Финальная мастер-версия: защищена от облачных сбоев сети и блокировок NCBI.
"""
    ...
    with tab_design:
        ...
        if seq_input:
            ...
            if not is_valid:
                ...
                # --- ОТОБРАЖЕНИЕ РЕЗУЛЬТАТОВ БЕЗ ОБРЕЗАНИЯ КОЛОНОК ---
                if st.session_state.primers_df is not None and not st.session_state.primers_df.empty:
                    ...
                    # Прямой вывод таблицы, подготовленной в utils.py, без урезания
                    st.dataframe(
                        df_disp,
        ...
        if blast_left_seq and blast_right_seq:
            ...
            if st.session_state.blast_results:
                ...
                for key, pair_results in st.session_state.blast_results.items():
                    ...
                    # ПРОВЕРКА НА ОШИБКИ И БАНЫ
                    errors = [res for res in pair_results if res.error_message]
                    if errors:
                        for res in errors:
                            st.error(f"**{res.primer_name}**: {res.error_message}")
                        st.warning("⚠️ Ошибка получения данных. Если проблема возникает только в облаке, возможно NCBI временно заблокировал IP-адрес сервера из-за высокого трафика. В таком случае используйте локальную версию приложения.")
                        continue
                    
                    if len(pair_results) == 2:
                        fwd_res, rev_res = pair_results[0], pair_results[1]
                        
                        # --- ПРОВЕРКА НА ПОЛНОЕ ОТСУТСТВИЕ СОВПАДЕНИЙ ---
                        if fwd_res.total_hits == 0 and rev_res.total_hits == 0:
                            st.warning("⚠️ NCBI BLAST не нашел **ни одного совпадения** для обоих праймеров. Убедитесь, что праймеры корректны, а база содержит целевой организм.")
                        else:
                            ...
                            if common_accs:
                                ...
                                st.info("ℹ️ Праймеры не образуют совместного продукта (ампликона) на известных матрицах в этой базе (возможно, они специфичны, но сидят на разных хромосомах/генах).")
