        ...
        if blast_left_seq and blast_right_seq:
            ...
            if st.button("🔍 Запустить BLAST", type="primary", use_container_width=True):
                if len(blast_left_seq) < 12 or len(blast_right_seq) < 12:
                    ...
                    # ДОБАВЛЕНО: Предупреждение о долгом поиске
                    ...
                    if total_queries > 4 or ("nt" in selected_dbs and "Все организмы" in selected_orgs_keys):
                        st.warning(f"⏳ **Внимание:** Вы выбрали сложный запрос ({total_queries} поисков, широкая база). Это может занять **от 5 до 15 минут**. Пожалуйста, дождитесь завершения и не закрывайте вкладку.")
                    ...
                    with st.spinner("Связь с серверами NCBI BLAST..."):
