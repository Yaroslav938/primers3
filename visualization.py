...
def create_primer_map(
    ...
    """Создаёт красивую интерактивную карту позиций праймеров."""
    if selected_pairs is None:
        selected_pairs = list(range(len(primers)))

    fig = go.Figure()

    # Шаблонная последовательность (основная ось)
    fig.add_trace(go.Scatter(
        x=[0, seq_length],
        y=[0, 0],
        mode="lines",
        line=dict(color="#B0BEC5", width=8),
        name="Матрица (Template)",
        hoverinfo="text",
        text=[f"Старт: 0 п.н.", f"Конец: {seq_length} п.н."],
    ))
    ...
    for idx, pair_idx in enumerate(selected_pairs):
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(symbol="triangle-right", size=12, color=color, line=dict(width=1, color="black")),
            name=f"Пара {pair_idx+1} Fwd",
            hovertext=f"<b>Forward #{pair_idx+1}</b><br>Позиция: {p.left_start} ➝ {p.left_start+p.left_length}<br>Tm: {p.left_tm:.1f}°C | GC: {p.left_gc:.1f}%<br>Seq: {p.left_seq}",
            hoverinfo="text",
        ))

        # Reverse
        fig.add_trace(go.Scatter(
            x=[p.right_start - p.right_length, p.right_start],
            y=[-y_offset, -y_offset],
            mode="lines+markers",
            line=dict(color=color, width=5),
            marker=dict(symbol="triangle-left", size=12, color=color, line=dict(width=1, color="black")),
            name=f"Пара {pair_idx+1} Rev",
            hovertext=f"<b>Reverse #{pair_idx+1}</b><br>Позиция: {p.right_start-p.right_length} ➝ {p.right_start}<br>Tm: {p.right_tm:.1f}°C | GC: {p.right_gc:.1f}%<br>Seq: {p.right_seq}",
            hoverinfo="text",
        ))
        ...
        fig.add_vrect(
            ...
            opacity=0.1,
            line_width=0,
            annotation_text=f"<b>{p.product_size} bp</b>",
    ...
    fig.update_layout(
        title=dict(text=title, font=dict(size=20, color="#1A73E8")),
        xaxis=dict(title="Позиция (п.н.)", showgrid=True, gridcolor="#ECEFF1", zeroline=False),
        yaxis=dict(showticklabels=False, showgrid=False, zeroline=True, zerolinecolor="#90A4AE", zerolinewidth=2),
        showlegend=True,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        height=350 + len(selected_pairs) * 40,
        plot_bgcolor="white",
        paper_bgcolor="white",
...
def render_blast_alignment_html(query_seq: str, subject_seq: str, q_start: int, s_start: int) -> str:
    """
    ...
    for q, s in zip(query_seq, subject_seq):
        if q == s:
            ...
            m_line += f"<span style='color:#608B4E;'>|</span>"
